    ...
    <div class="p-6">
            ...
            <div class="flex flex-col justify-center mt-6 p-6 space-y-12">
                ...
                <section>
                    ...
                    <a href="{{ route('news.show', $recentNews) }}">
                        <div
                            class="flex flex-col justify-center items-center lg:justify-between gap-x-5 lg:flex-row bg-white p-4 rounded-lg hover:scale-105 transition duration-[0.3s] ease-out shadow-lg">
                            {{--Zdjęcie--}}
                            @if($recentNews->image_path)
                                <img alt="{{ $recentNews->slug }}" src="{{ $recentNews->getURLImage() }}"
                                     class="mx-auto rounded-lg w-full sm:w-1/2 lg:w-auto lg:h-64">
                            @else
                                <div class="flex items-center justify-center w-full sm:w-1/2 lg:w-1/3">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="1.5" stroke="currentColor" class="text-gray-800 size-24">
        ...
        @else
            <div class="flex items-center justify-center h-[20vh]">
                <h1 class="text-4xl text-center">Aktualnie nie odnaleziono żadnych ogłoszeń. <br> Przepraszamy za
                    utrudnienia. &#128575;</h1>
            </div>
        @endif
    </div>
