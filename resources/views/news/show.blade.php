<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row p-6 gap-x-4">
        <article class="md:w-3/4 p-4 md:border-r-2 md:border-gray-300/50">
            <div class="">
                <p class="lg:text-right text-green-600">Dodano: &nbsp;<time>{{ $news->formatedDate() }} <br> O godzinie: {{ $news->formatedTime() }}</time></p>
            ...
            @foreach($newsCollection as $singleNews)
                <a href="{{ route('news.show', $singleNews) }}">
                    <div class="bg-white rounded-md h-72 md:h-64 w-auto p-2">
                        ...
                        <p class="text-sm text-green-600 text-center mt-4">Dodano: &nbsp;<time>{{ $news->formatedDate() }} </br> O godzinie: {{ $news->formatedTime() }}</time></p>
                    </div>
                </a>
            @endforeach
        </aside>
