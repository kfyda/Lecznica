<nav class="fixed top-0 z-50 bg-black w-full">
        ...
        <div class="flex justify-between items-center h-20 px-4">
            ...
            <div class="hidden space-x-8 sm:flex sm:items-center">
                ...
                <x-dropdown align="left" width="48" :active="request()->routeIs('services.*')" class="text-white hover:text-green-300">
                    <x-slot name="trigger">
                        <button wire:click="toggleDropdown" class="text-md leading-5 font-medium text-white bg-transparent hover:text-green-300 focus:outline-none transition ease-in-out duration-150">
                            <div>Usługi</div>
                        </button>
                    </x-slot>
            ...
            <div class="flex items-center sm:hidden">
                <button wire:click="toggle" class="inline-flex items-center justify-center p-2 rounded-md text-white hover:text-green-300 transition duration-150 ease-in-out">
                    <svg class="h-6 w-6" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                        <path x-show="!$wire.open" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                        <path x-show="$wire.open" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            </div>
        </div>
    ...
    <div x-show="$wire.open" class="absolute top-0 left-0 w-full bg-black bg-opacity-90 sm:hidden">
        <div class="space-y-1 px-4 py-2">
            ...
            <x-responsive-dropdown x-data="{ openDropdown: @entangle('openDropdown') }" :active="request()->routeIs('services.*')" class="text-white hover:text-green-300">
                <x-slot:trigger>
                    <button wire:click="toggleDropdown" class="flex items-center">
                        <div>Usługi</div>
                    </button>
                </x-slot:trigger>
                ...
                <x-slot:content>
                    @foreach($services as $service)
                        <x-responsive-nav-link :href="route('services.show', $service)" :active="request()->routeIs('services.show', $service)" class="text-white hover:text-green-300">
                            {{ $service->name }}
                        </x-responsive-nav-link>
                    @endforeach
                </x-slot:content>
        ...
        <div class="pt-4 pb-1 border-t bg-gray-800 border-gray-600">
            <div class="px-4">
                <div class="font-medium text-base text-white">{{ $user->name ?? 'Gość' }}</div>
                <div class="font-medium text-sm text-gray-400">{{ $user->email ?? 'gosc@test.pl' }}</div>
            </div>
